...
sigma = unp.uarray([Sigma_zink.n, Sigma_gallium.n, Sigma_brom.n, Sigma_rubidium.n, Sigma_strontium.n, Sigma_zirkonium.n ],
[Sigma_zink.s, Sigma_gallium.s, Sigma_brom.s, Sigma_rubidium.s, Sigma_strontium.s, Sigma_zirkonium.s ])
sigma_n = unp.nominal_values(sigma)
Ord = np.array([30, 31, 35, 37, 38, 40]) 
E_k = unp.uarray([Ezink.n, Egallium.n, Ebrom.n, Erubidium.n, Estrontium.n, Ezirkonium.n], [Ezink.s, Egallium.s, Ebrom.s, Erubidium.s, Estrontium.s, Ezirkonium.s])
E_k_n = unp.nominal_values(E_k)
xplot = np.linspace(650, 1300, 1000)

params , ma = np.polyfit((Ord - sigma_n)**2 ,E_k_n, deg =1, cov = True)
errors = np.sqrt(np.diag(ma))
a_ryd = ufloat(params[0], errors[0])
c_ryd = ufloat(params[1], errors[1])
print('Rh', a_ryd)
print('c', c_ryd)
print('dif', 1 - 13.6/a_ryd.n)

##PLOTS
plt.figure()
plt.plot((Ord - sigma_n)**2, E_k_n, 'k.', label='Errechnete Punkte')
plt.plot(xplot, a_ryd.n * xplot + c_ryd.n, 'g-', label='Ausgleichsgerade')
plt.xlabel(r'$(z -  \sigma)^2 $')
plt.ylabel(r'$ E_{\text{K}} \quad [\si{\electronvolt}]$')
plt.grid()
plt.legend()
plt.tight_layout()
